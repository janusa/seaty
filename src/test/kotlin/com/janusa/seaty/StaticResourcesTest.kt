...
class StaticResourcesTest : AbstractWebIntegrationTest() {
    ...
    fun `serves the stylesheet as css`() {
        restClient.get().uri("/css/styles.css")
            .exchange()
            .expectStatus().isOk()
            .expectHeader().contentTypeCompatibleWith(css)
    }

    @Test
    fun `index page has the expected structure and a working stylesheet link`() {
        val html = restClient.get().uri("/index.html")
            .exchange()
            .expectStatus().isOk()
